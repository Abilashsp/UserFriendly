...
function App() {
  ...
  const handleEnter = () => {
    if (InputValue.trim() !== "") {
      ...
      setCenterbarValues((prevValues) => [
        ...
        const menuItem = list.find((menu) => menu.name === textid);
    
          const menuItemIndex = list.indexOf(menuItem);
          dispatch(addChild(menuItemIndex, InputValue));
          setInputValue("");
      ...
      setCenterbarValues((prevValues) =>
        prevValues.map((item) => {
          if (item.id === textid) {
            ...
            if (_textColor) newItem.textColor = _textColor.color;
            if (_textBgColor) newItem.textBgColor = _textBgColor.color;
            if (_textalign) newItem.textalign = _textalign.class;
  
            if (_text) {
              if (!newItem.textStyle) newItem.textStyle = [];
              if (newItem.textStyle.includes(_text.class)) {
                newItem.textStyle = newItem.textStyle.filter(
                  (val) => val !== _text.class
                );
              } else {
                newItem.textStyle.push(_text.class);
              }
            }
  
            if (
              newItem.textColor !== item.textColor ||
              newItem.textBgColor !== item.textBgColor ||
              newItem.textalign !== item.textalign ||
              (newItem.textStyle && !_.isEqual(newItem.textStyle, item.textStyle))
            ) {
              setHistory((prevHistory) => [
                ...prevHistory,
                {
                  ...
                  changes: {
                    ...
                    textBgColor: newItem.textBgColor,
                    textalign: newItem.textalign,
                    textStyle: newItem.textStyle,
                  },
                },
              ]);
            }
  ...
  {history.map((change, index) => (
    <div key={index} className="text-slate-900 flex justify-between border-2">
      <div>
        <p>TextLabelName: {change.itemValue}</p>
        {Object.keys(change.changes).map((property) => (
          <p key={property}>
            {property}: {JSON.stringify(change.changes[property])}
          </p>
        ))}
      </div>
    </div>
