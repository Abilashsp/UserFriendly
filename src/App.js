  ...
  const handleEnter = () => {
    if (InputValue.trim() !== "") {
      ...
      setCenterbarValues((prevValues) => [
       ...
       const newHistory = [...history.slice(0, currentIndex + 1), centerbarValues];
    ...
    <div key={index} className="text-slate-900">
      {items.map((item) => (
        <div key={item.id}>
          ...
          <p>Text Align: {item.textalign}</p>
          <p>Text Color: {item.textColor}</p>
          <p>Text Background Color: {item.textBgColor}</p>
